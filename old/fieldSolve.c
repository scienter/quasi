...
void solveLaser(Domain *D)
{
  ...
  while(L->next)  {
    ...
    for(i=istart; i<iend; i++)  {
      ...
      D->aNext[i-1][j][k].real=(D->aOld[i][j][k].real-D->aOld3[i][j][k].real)*alpha1-(D->aOld[i][j][k].img-D->aOld3[i][j][k].img)*beta1+D->aNow[i-1][j][k].real*alpha2-D->aNow[i-1][j][k].img*beta2+D->aOld[i-1][j][k].real*alpha3-D->aOld[i-1][j][k].img*beta3;
      D->aNext[i-1][j][k].img=(D->aOld[i][j][k].real-D->aOld3[i][j][k].real)*beta1+(D->aOld[i][j][k].img-D->aOld3[i][j][k].img)*alpha1+D->aNow[i-1][j][k].real*beta2+D->aNow[i-1][j][k].img*alpha2+D->aOld[i-1][j][k].real*beta3+D->aOld[i-1][j][k].img*alpha3;
    }
